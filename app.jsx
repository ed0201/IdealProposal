    ...
    <header className="bg-[var(--bone)] border-b border-[var(--line)] sticky top-0 z-30">
      <div className="px-6 lg:px-10 py-5 flex items-center justify-between">
        ...
        <div className="flex items-baseline gap-3">
          <div className="font-display text-2xl tracking-wide text-[var(--ink)]">IdealProposal</div>
        ...
        <nav className="flex items-center gap-8 border-b border-transparent">
          {[
            { id: "configurator", label: "Configurador Interactivo" },
            { id: "education",    label: "Guía de Educación" },
          ].map(t => (
            <button key={t.id}
              onClick={() => setTab(t.id)}
              data-active={tab === t.id}
              className="tab-btn py-2 text-[11px] tracking-luxe uppercase text-[var(--ink)] hover:text-black transition"
